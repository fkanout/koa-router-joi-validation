    ...
    try {
      if (_config.schema === true && schema) {
        ...
        if (params && !_config.alternate.includes("params")) {
          await handleErrorWithSource(
            "params",
            _joi.default.object(params).validateAsync(ctx.params)
          );
        }
      ...
      if (inputs[ctx.status]) {
        await handleErrorWithSource(
          "output",
          _joi.default.object(inputs[ctx.status]).validateAsync(ctx.body)
        );
      }
    } catch (error) {
